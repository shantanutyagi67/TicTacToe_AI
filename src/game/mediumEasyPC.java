...
public class mediumEasyPC extends JComponent implements Runnable, MouseListener, KeyListener{
	...
	public static void main(String args[]) {
		...
		frame.getContentPane().add(new mediumEasyPC());
...
//		scl+=0.001;
	...
	private void computer() {
		...
		for(int j=0;j<3;j++)
			for(int k=0;k<3;k++)
				if(board[j][k]=="") cnt++;
		int randum = (new Random()).nextInt(100)+1;
		int i=-1,j=-1;
		if(randum>=1&&randum<=20) {//center
			i=1;
			j=1;
		}
		else if(randum>=21&&randum<=68) {//corner
			if(randum>=21&&randum<=32) {
				i=0;
				j=0;
			}
			else if(randum>=33&&randum<=44) {
				i=0;
				j=2;
			}
			else if(randum>=45&&randum<=56) {
				i=2;
				j=0;
			}
			else if(randum>=57&&randum<=68) {
				i=2;
				j=2;
			}
		}
		else if(randum>=69&&randum<=100) {//edge
			if(randum>=69&&randum<=76) {
				i=0;
				j=1;
			}
			else if(randum>=77&&randum<=84) {
				i=1;
				j=0;
			}
			else if(randum>=85&&randum<=92) {
				i=1;
				j=2;
			}
			else if(randum>=93&&randum<=100) {
				i=2;
				j=1;
			}
		}
